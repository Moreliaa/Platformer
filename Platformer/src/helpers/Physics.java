...
public class Physics {
	...
	/**
	 * Returns true if the AABB of the given coordinates intersects with the given tile, otherwise false.
	 */
	public static boolean checkCollision(
			float x1, float y1, int width1, int height1, Tile t) {
		if(x1 + width1 > t.getX() && x1 < t.getX() + tileSize &&
				y1 + height1 > t.getY() && y1 < t.getY() + tileSize)
				return true;
		return false;
	}
	
	/**
	 * Returns true if the two AABBs of the given coordinates intersect with each other, otherwise false.
	 */
	public static boolean checkCollision(
	...
	private static float setInitialSmallJumpVelocity() {
		return (-1) * (float) Math.sqrt(Math.pow(initialJumpVelocity, 2)
						+ (-1) * 2 * gravity * (maxJumpHeight - minJumpHeight));
	}
